...
import {
  getAllPlayerTransfersRepo,
  getPlayerTransfersRepo,
} from "../repositories/player-transfers.repo";
import { idSchema } from "../validations/primitives.schema";

export async function getAllPlayerTransfersService(
  query: unknown,
): Promise<PlayerTransferListItem[]> {
  const parsedQuery = playerTransfersQuerySchema.parse(query);

  return getAllPlayerTransfersRepo(parsedQuery);
}
